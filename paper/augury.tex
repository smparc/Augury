...
\node[hyp, right=13mm of markets] (posts) {\textbf{posts} \hfill \textit{1 d}\\ \underline{post\_id, market\_id, created\_at}\\ followers, engagements\\ minhash[], lsh\_bucket\\ filter\_verdict, source};
...
\node[hyp, below=5mm of prices] (signals) {\textbf{signals} \hfill \textit{7 d}\\ \underline{market\_id, ts, model\_version}\\ s\_t, p\_hat, calibration\\ half\_life\_seconds};
...
\end{tikzpicture}
\caption{The shared schema. Underlined fields are the primary key. Every
...
\end{tikzpicture}
\caption{The Dagster asset graph, as declared in
...
\node[font=\footnotesize, augGrey, anchor=west] at (axis cs:8.2,1e-250) {representable};
\end{semilogyaxis}
\end{tikzpicture}
\caption{The decay kernel over 32 days. At a 6-hour half-life nothing comes close
...
\end{axis}
\end{tikzpicture}
\caption{Effective half-life as a function of the two adaptive triggers,
...
\end{tikzpicture}
\caption{The filter's decision order. Cheap field checks precede the MinHash
...
% lifelines
\foreach \x/\name in {0/{Worker A}, 3.6/{\code{read\_budget}}, 7.2/{Worker B}} {
  \node[draw, thick, fill=augGrey!8, minimum width=20mm, minimum height=5mm] (h\x) at (\x,0) {\name};
  \draw[augGrey!60, dashed] (\x,-0.35) -- (\x,-5.0);
}
\def\naive{-0.9}
% Naive read-then-write
\node[anchor=west, font=\footnotesize\bfseries, augRed] at (-1.9,\naive+0.35) {Read-then-write (racy)};
\draw[->, augRed] (0,\naive) -- node[above, font=\scriptsize] {SELECT} (3.6,\naive);
\draw[->, augRed] (7.2,\naive-0.45) -- node[above, font=\scriptsize] {SELECT} (3.6,\naive-0.45);
\node[annot, text=augRed, anchor=west] at (3.75,\naive-0.9) {both read \code{1900/2000} --- both see headroom};
\draw[->, augRed] (0,\naive-1.35) -- node[above, font=\scriptsize] {+500} (3.6,\naive-1.35);
\draw[->, augRed] (7.2,\naive-1.8) -- node[above, font=\scriptsize] {+500} (3.6,\naive-1.8);
\node[annot, text=augRed, anchor=west] at (3.75,\naive-2.2) {\textbf{2900 / 2000} --- ceiling breached};

\def\safe{-3.75}
\node[anchor=west, font=\footnotesize\bfseries, augGreen] at (-1.9,\safe+0.35) {Single conditional upsert};
\draw[->, augGreen] (0,\safe) -- node[above, font=\scriptsize] {\code{INSERT..ON CONFLICT}} (3.6,\safe);
\draw[->, augGreen] (7.2,\safe-0.45) -- node[above, font=\scriptsize] {\code{INSERT..ON CONFLICT}} (3.6,\safe-0.45);
\draw[<-, augGreen, dashed] (0,\safe-0.9) -- node[above, font=\scriptsize] {\code{RETURNING 2400}} (3.6,\safe-0.9);
\draw[<-, augRed, dashed] (7.2,\safe-1.25) -- node[above, font=\scriptsize] {\code{0 rows} $\Rightarrow$ \code{BudgetExceeded}} (3.6,\safe-1.25);
\end{tikzpicture}
\caption{Why the reservation must be one statement. The \code{WHERE} clause
...
\node[annot, anchor=west] at (axis cs:6000,0.86)
  {$b=312$: \ $1{,}035$ shares reach $0.50$};
\node[annot, anchor=west] at (axis cs:96000,0.28)
  {$b=225{,}792$: \ $748{,}902$ shares needed};
\end{axis}
\end{tikzpicture}
\caption{What the $723\times$ ambiguity actually costs. Each curve is the same
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[font=\footnotesize, x=0.115mm, y=1cm]
\foreach \i/\o in {0/0, 1/24, 2/48, 3/72} {
  \pgfmathsetmacro{\yy}{-\i*0.62}
  \draw[fill=augBlue!18, draw=augBlue, thick] (\o*1,\yy) rectangle (\o*1+72,\yy+0.42);
  \node[font=\scriptsize, augBlue!60!black] at (\o*1+36,\yy+0.21) {train (72)};
  \draw[fill=augOrange!22, draw=augOrange, thick] (\o*1+72,\yy) rectangle (\o*1+96,\yy+0.42);
  \node[font=\scriptsize, augOrange!60!black] at (\o*1+84,\yy+0.21) {test (24)};
  \node[anchor=east, font=\scriptsize, augGrey] at (-4,\yy+0.21) {window \i};
  % fitted params
  \node[anchor=west, font=\scriptsize, augGrey] at (\o*1+100,\yy+0.21)
    {$b_{\i}$, $\rho_{\i}$ fitted on train};
}
\draw[{Latex[length=1.8mm]}-{Latex[length=1.8mm]}, augGrey] (0,-2.15) -- (96,-2.15)
  node[midway, below, font=\scriptsize, augGrey] {step $=$ test size $\Rightarrow$ test windows never overlap};
\draw[augGrey, dashed] (72,0.5) -- (72,-2.0);
\node[anchor=south, font=\scriptsize, augGrey, align=center] at (170,0.48)
  {calibration uses training-window books only};
\end{tikzpicture}
\caption{Walk-forward evaluation in the C++ engine. Each window fits $b$ from its
...
\end{axis}
\end{tikzpicture}
\caption{Mapping the signal into probability space. The affine map is a straight
...
\begin{table}[t]
\centering
\caption{The same command, the same market, six days apart. Only the venue data
...
Best bid $\times$ depth   & $0.03 \times 93$ & $0.01 \times 98{,}353$ \\
Best ask $\times$ depth   & $0.04 \times 67{,}296$ & $0.02 \times 858{,}548$ \\
Logit width               & $0.2980$ & $0.7033$ \\
\midrule
$b$ (bid side)            & $312$ & $139{,}846$ \\
$b$ (mean)                & $113{,}052$ & $680{,}294$ \\
...
smoothly to $-0.369$ at lag $+6$ --- a clean, entirely spurious signal-leads-price
...
\addplot[augBlue] table[x=hour, y=p_hat, col sep=comma] {data/series.csv};
\addlegendentry{$\hat{p} = (S(t)+1)/2$ \ (signal)}
\addplot[augGreen] table[x=hour, y=sim, col sep=comma] {data/series.csv};
...
\end{axis}
\end{tikzpicture}
\caption{The 3 August run, plotted from \code{paper/data/series.csv}. The signal
...
\addplot[fill=augRed!55, draw=augRed] coordinates
  {(0,-0.1688) (1,-0.1793) (2,-0.1912) (3,-0.2048)};
\addplot[augGrey, sharp plot, update limits=false] coordinates {(-3.7,0) (3.7,0)};
...
\end{axis}
\end{tikzpicture}
\caption{Cross-correlation of $S(t)$ against price, both runs, lags $-3$ to $+3$.
...
\begin{semilogxaxis}[
  ...
  ymin=0.4, ymax=4.6,
  xbar, bar width=8pt,
  nodes near coords, every node near coord/.append style={font=\scriptsize, /pgf/number format/fixed, /pgf/number format/1000 sep={,}},
]
\addplot[fill=augOrange!65, draw=augOrange] coordinates {(858548,1) (98353,2)};
\addplot[fill=augBlue!60, draw=augBlue] coordinates {(67296,3) (93,4)};
\end{semilogxaxis}
\end{tikzpicture}
\caption{Resting depth on each side of the book, both observations, on a log
...
====================================================================
  ...
  target: The Federal Reserve will cut interest rates by 25 basis points at
...
-- market data ------------------------------------------------------
  ...
  depth (bid/ask)      : 98353.33 / 858548.26
  LMSR b (from depth)  : 680,294.47
...
-- synthetic LMSR market --------------------------------------------
  sim ticks            : 48
  sim price            : 0.0890 -> 0.2204
  real market mid      : 0.0150  (gap +0.2054)

-- lead-lag ---------------------------------------------------------
  strongest |r|        : -0.3686 at lag +6h (signal leads, n=33)
    lag +0h  r = -0.1688  (contemporaneous)
    lag +1h  r = -0.1793  (signal leads)
    lag +2h  r = -0.1912  (signal leads)
    lag +3h  r = -0.2048  (signal leads)
  Granger              : market price is nearly constant over this window
...
reports a monotone gradient to $-0.369$ labelled \emph{signal leads} --- on the
