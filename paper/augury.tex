...
verification: three independent implementations of the shared mathematics are
pinned to 53 golden vectors at $10^{-9}$; the numerically fragile paths are
analysed rather than asserted. That analysis produces three corrections to the
system's own documented claims --- the float64 underflow threshold for the
decay kernel is $\lambda\Delta t > 745$, not the $233$ claimed; the shipped LSH
band configuration reaches only $20.4\%$ recall at its own declared duplicate
threshold; and the per-step liquidity recalibration described in the
simulation docstring is provably a no-op as implemented. No lead--lag result is
reported, because none is yet earned: the ingester has not run long enough. The
contribution is the instrument and its calibration, not a finding.
\end{abstract}
...
Each of these is enforced against in code rather than left to the analyst's
...
\item \textbf{Cross-language verification} (Section~\ref{sec:golden}): 53 golden
      vectors generated from the Python reference, asserted at $10^{-9}$ by
      independent implementations in R and C++.
...
\begin{table}[t]
\centering
\caption{Services, responsibilities, and verification status. Line counts are
...
\midrule
\code{augury-signal}    & Python 3.12 & 7{,}545 & NLP, $S(t)$, orchestration & runs, 139 tests \\
...
\code{augury-analytics} & R 4.6       &   981   & Granger, Brier, reports    & runs, 55 tests \\
\midrule
Schema / migrations     & SQL, JSON   & 1{,}200 & Shared contracts           & applied \\
\bottomrule
\end{tabular}
\end{table}
...
The module documentation motivates this by claiming that at a 30-minute
...
because it changes where the guard actually binds. IEEE-754 binary64 has
subnormals down to $\approx 4.94 \times 10^{-324}$, so $\exp$ returns zero only
for arguments below $\ln(4.94\times10^{-324}) \approx -745.1$. Concretely,
$e^{-233} = 7.14\times10^{-102}$, a perfectly ordinary subnormal-free double.
Table~\ref{tab:underflow} gives the real boundary.

\begin{table}[t]
\centering
\caption{Decay kernel magnitude by post age and half-life. Underflow to exactly
zero begins at $\lambda \Delta t > 745.1$.}
\label{tab:underflow}
\small
\begin{tabular}{@{}llrl@{}}
\toprule
...
half-life, giving $\lambda \Delta t = 998$. Every term in both sums underflows
to zero, the naive ratio is $0/0$, and Eq.~\ref{eq:logspace} returns the correct
answer of $0.5$ --- the stance of the only post present. What the code does is
right; only the stated threshold was off by a factor of three, and a reader
calibrating a shorter half-life from the documented figure would have
over-estimated the danger zone.
...
parameter moved is not comparable to one whose did not, and the column is what
makes that recoverable after the fact.
...
price the same distance costs
$b\,(\operatorname{logit}(a) - \operatorname{logit}(d))$ shares for bid $d$ and
ask $a$. Setting the two equal:

\begin{equation}
b = \frac{\text{depth}}{\operatorname{logit}(a) - \operatorname{logit}(d)}.
\label{eq:calib}
\end{equation}
...
asymmetric and its level is near-certainly non-stationary in the sense the test
assumes. Both series are mapped through
...
capped at $\lfloor (n-2)/4 \rfloor$ so the VAR is never fit on fewer effective
...
spurious regression. The ADF null is a unit root, so a \emph{small} $p$-value
means stationary --- the opposite of the intuition most readers carry into it,
which is why the R implementation returns the result as a named list rather than
a bare number.
...
\midrule
\code{signal}                & $S(t)$ aggregation           & 7 & \code{extreme\_age}: $\lambda \Delta t = 998$ \\
...
\code{calibration}           & affine, logit, sigmoid        & 21 & $\sigma(\pm 30)$; $\operatorname{logit}(0.001)$ \\
\midrule
...
vectors include cases chosen specifically to be hostile --- a 30-day-old post at
a 30-minute half-life, a crossed book that must fall back rather than raise,
$\sigma(\pm 30)$ where a naive implementation loses all precision on one branch.
And the C++ suite locates the file by walking up from the build directory, so
the gate cannot be accidentally skipped by a build layout change; a missing file
fails the suite rather than silently passing zero assertions.
...
ADF $p$-values       & signal $0.144$ (non-stationary), price $0.025$ \\
...
adjustment across every market before it means anything. The ADF result reports
the signal series as non-stationary at $p = 0.144$, which is itself a reason to
distrust the $F$ statistic printed two lines above it --- the guard fired and
said so rather than being silently satisfied.
...
Section~\ref{sec:econ} predicts it will do on an out-of-the-money market ---
mapping a mildly bearish lexicon signal onto a probability far above the market's
own, because the affine map's implicit claim that $S = 0$ means $50/50$ is a
strong and here plainly false assumption. This is precisely the case Platt
...
Rust \code{ReadBudget::}\-\code{reserve} &
...
\subsection{One defect that is documented as fixed but is not}

The review also recorded a fifth item --- that Python's \code{simulate\_lmsr}
``recalibrated $b$ to the same constant every step, making per-step liquidity
tracking a no-op, and left a dead local behind'' --- and the project
documentation lists it among the defects found \emph{and fixed}. It is not
fixed, and the distinction matters because the function's own docstring makes a
...
with that same unchanged value. By the invariance property established in
Section~\ref{sec:lmsr} --- recalibration rescales $\mathbf{q}$ to hold
$\operatorname{logit}(p) = (q_{\text{yes}} - q_{\text{no}})/b$ fixed --- calling
\code{recalibrate} with an unchanged $b$ has a scaling ratio of exactly $1$ and
is therefore provably a strict no-op on every field of the market state. The
dead local likewise survives; it is now explicitly deleted with
\code{del ordered\_prices} at the end of the function, which suppresses the
linter without removing the dead computation.
...
\code{augury-ingest} & cargo    & 47 tests  & \textbf{never compiled} \\
...
code is unverified code. The 47 Rust and 26 C++ test cases in
...
implementations pinned to a shared set of hostile golden vectors; a set of
statistical guards that are enforced in code rather than in the analyst's
memory; and an evaluation path on which the answer ``the market is already
...
pipeline stores only one book snapshot per cycle. None of these were found by
the test suite. Two of them were found by working the arithmetic out on paper
and checking it against the code, which is an argument for writing the paper.
