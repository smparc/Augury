...
A second pass has since closed five of the gaps this section used to list. Each
is recorded below with the property that now holds and the test that pins it.

| Where | Was | Now |
|---|---|---|
| `slice._analyze` | Degeneracy tested by `len(set(prices))`, so a price pinned at one tick still counted as "two distinct values" and the cross-correlation ran against 6-ULP rounding residue | Tested on the price **range** against a `1e-9` floor, in both guards. `test_a_price_pinned_to_one_tick_yields_no_correlation` reproduces the original artifact; `test_a_genuine_one_tick_move_is_still_measured` proves the floor does not swallow a real move |
| `steps.simulate_lmsr` | Docstring claimed per-step recalibration; `b` was computed once and `recalibrate(b)` was a provable no-op | Recalibrates from the last book **at or before** each step, by binary search. Books only — candles carry quotes but no depth. Degrades to a single constant `b` when only one snapshot exists, which is the honest behaviour |
| `minhash.rs` banding | 8 bands × 16 rows retrieved **20.4%** of true near-duplicate pairs at the 0.80 threshold — four spam-ring members in five were never compared | 16 bands × 8 rows: **94.7%** at the threshold, 0.02% at 0.30. `banding_retrieves_the_overwhelming_majority_at_the_threshold` asserts the property rather than the constant. Bucket keys now carry their own row width (`r8:…`), so re-tuning needs no data migration |
| `minhash.rs` lookup | Every post was compared against the **whole** window, with buckets used only to skip the similarity call — ~10⁶ string comparisons per post at the shipped capacity, despite the docstring claiming linearity | A real inverted index, bucket → arrival number, unlinked on eviction. `the_lsh_index_is_bounded_too` pins that the index cannot outgrow the window |
| R `granger_test` | `liquidity` was accepted, recorded as applied in `granger_results.liquidity_control`, and then **ignored** — a run could claim controls it never used | Controls enter both nested models; the flag is set from what actually entered. `lmtest::grangertest` replaced by an explicit `anova` of the two fits, with a test pinning bit-identical output on the uncontrolled path |
| `clients/x.py` | `FixtureXBackend` recomputed its `now()` anchor **inside** `search`, so one backend replayed the same fixture differently on every call | Anchor fixed at construction. `test_repeated_reads_replay_identically` pins it |

Remaining, stated explicitly rather than left to be discovered:

- **The liquidity control is wired in R but not in Python.** `augury-analytics` is the authoritative implementation and now does the controlled test properly; `augury_signal.analytics.leadlag` still runs the bare two-variable version, which is what the vertical slice reports. The two therefore answer slightly different questions on the same data, and the slice's Granger line should be read as indicative.
- ~~**Rust and C++ have never been compiled.**~~ **Resolved.** Both build and pass their suites (42 cargo tests; 97 Catch2 assertions in 26 cases). Note that the Rust and R changes in the table above were written on a machine with **no MSVC linker and no R runtime**, so they are reviewed but not executed; the Python changes are verified (143 tests pass).
